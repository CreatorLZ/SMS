...
exports.deleteAttendance = exports.getCalendarAttendance = exports.updateAttendance = exports.getAttendanceHistory = exports.getStudentAttendance = exports.getClassAttendance = exports.markAttendance = void 0;
    ...
    try {
        ...
        if (existingAttendance) {
            return res.status(400).json({
                message: "Attendance already exists for this date. Use update endpoint instead.",
            });
        }
...
const getStudentAttendance = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const attendance = yield Attendance_1.Attendance.find(query)
            .populate("classroomId", "name")
            .populate("records.studentId", "fullName studentId")
            .sort({ date: -1 })
            .skip(skip)
            .limit(parseInt(limit));
        const total = yield Attendance_1.Attendance.countDocuments(query);
        // Extract student's records from attendance
        const studentRecords = attendance
            .map((att) => {
            const studentRecord = att.records.find((record) => record.studentId.toString() === studentId);
            if (!studentRecord)
                return null;
            return {
                _id: att._id,
                classroomId: att.classroomId,
                classroomName: att.classroomId.name,
                date: att.date,
...
const getAttendanceHistory = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { classroomId, studentId, startDate, endDate, page = 1, limit = 10, period, } = req.query;
        if (!req.user) {
            return res.status(401).json({ message: "User not authenticated" });
        }
        // Build query
...
const updateAttendance = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Verify classroom exists and user has access
        const classroom = yield Classroom_1.Classroom.findById(attendance.classroomId);
        if (!classroom) {
            return res.status(404).json({ message: "Classroom not found" });
        }
        // Check permissions
...
const getCalendarAttendance = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        attendanceRecords.forEach((record) => {
            ...
            const dateKey = `${record.date.getFullYear()}-${String(record.date.getMonth() + 1).padStart(2, "0")}-${String(record.date.getDate()).padStart(2, "0")}`;
            const stats = {
                present: 0,
                absent: 0,
                late: 0,
...
exports.getCalendarAttendance = getCalendarAttendance;
...
const deleteAttendance = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const attendance = yield Attendance_1.Attendance.findOneAndDelete({
            classroomId,
            date: new Date(date),
        });
        if (!attendance) {
