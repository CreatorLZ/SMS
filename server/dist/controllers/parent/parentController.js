            ...
            if (student.classroomId) {
                ...
                if (activeTerm) {
                    ...
                    const holidays = activeTerm.holidays.reduce((sum, holiday) => {
                        return (sum +
                            Math.ceil((holiday.endDate.getTime() - holiday.startDate.getTime()) /
                                (1000 * 60 * 60 * 24)));
                    }, 0);
...
const getChildAttendance = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const allAttendanceRecords = yield Attendance_1.Attendance.find(Object.assign(Object.assign({ classroomId: student.classroomId }, dateFilter), { "records.studentId": studentId })).select("records");
        let totalDays = 0;
        let presentDays = 0;
        let absentDays = 0;
        let lateDays = 0;
...
const getChildResults = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const results = student.results.map((result) => ({
            ...
            overallAverage: result.scores.reduce((sum, score) => sum + score.totalScore, 0) /
    ...
    try {
        ...
        const childrenAttendance = yield Promise.all(linkedStudents.map((student) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (student.classroomId) {
                ...
                if (activeTerm) {
                    const termStart = activeTerm.startDate;
                    const termEnd = activeTerm.endDate;
                    const totalDaysCalc = Math.ceil((termEnd.getTime() - termStart.getTime()) / (1000 * 60 * 60 * 24));
        ...
        const summary = {
            ...
            excellentCount: childrenAttendance.filter((c) => c.status === "excellent")
                .length,
            goodCount: childrenAttendance.filter((c) => c.attendance >= 85 && c.attendance < 95).length,
