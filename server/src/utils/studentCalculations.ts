import { calculateSchoolDays } from "./schoolDays";
 ...
 */
export const calculateGPA = (student: any): number => {
  if (!student.results || student.results.length === 0) return 0;

  let validResultsCount = 0;
  const totalScore = student.results.reduce((sum: number, result: any) => {
    if (result.scores && result.scores.length > 0) {
      const subjectAverage =
        result.scores.reduce((subjectSum: number, score: any) => {
...
export const calculateAttendance = (
  ...
  percentage: number;
  breakdown?: { present: number; absent: number; late: number; total: number };
} => {
  ...
  for (
    let d = new Date(activeTerm.startDate);
    d <= activeTerm.endDate;
